...
Each provider received the same company domain through its documented company or funding endpoint. Every completed call produced a local checkpoint with status, latency, failure reason, normalized funding fields, and relevant response paths. Existing checkpoints are skipped by default. Only explicitly selected retry statuses can be retried.
